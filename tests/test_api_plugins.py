...
def make_app(db_path, plugins_config=None, job_scheduler=None):
    app = FastAPI()
    ...
    app.state.config = {
        ...
        # No NGU_PLUGIN_DIR set for these tests -- discovery uses only the
        # real built-in plugins (currently just makerworld).
        "plugin_dir": None,
    }
...
def test_plugins_error_status_and_last_error_from_latest_run(tmp_path):
